...
def main():
    file = open('conditionsMixMADDPG.json', 'r')
    ...
    switchIntervalLevels = condition['switchIntervalLevels']

    startTime = time.time()
    fileName = 'mixTrainMADDPG.py'
    numSample = None
    ...
    conditionLevels = [(mapSize, colorA, colorB, maxEpisode, maxTimeStep, bufferSize, minibatchSize, learningRateActor,
                        learningRateCritic, gamma, tau, learnInterval, switchInterval)
                       ...
                       for switchInterval in switchIntervalLevels]

    conditions = []
    for condition in conditionLevels:
        mapSize, colorA, colorB, maxEpisode, maxTimeStep, bufferSize, minibatchSize, learningRateActor, learningRateCritic, gamma, tau, learnInterval, switchInterval = condition
        parameters = {'mapSize': mapSize, 'colorA': colorA, 'colorB': colorB, 'maxEpisode': maxEpisode, 'maxTimeStep': maxTimeStep,
                      ...
                      'learningRateCritic': learningRateCritic, 'gamma': gamma, 'tau': tau, 'learnInterval': learnInterval, "switchInterval": switchInterval}
