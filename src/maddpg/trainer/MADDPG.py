...
class ActOneStepOneHot:
    def __init__(self, actByTrainNoisy):
        self.actByTrain = actByTrainNoisy

    def __call__(self, model, allAgentsStatesBatch):
        allAgentsStates = np.array(allAgentsStatesBatch)#.reshape(-1, 2)
        action = self.actByTrain(model, allAgentsStates)[0]
        action_oneHot = np.zeros_like(action)
        action_oneHot[action.argmax()] = 1
...
def actByPolicyTrainNoisy(model, allAgentsStatesBatch):
    ...
    stateDict = {agentState_: allAgentsStatesBatch[i][None] for i, agentState_ in enumerate(allAgentsStates_)}
