...
class PrintPreface:
    ...
    def __call__(self, winReward):
        ...
        preface3 = fontPreface.render('Reward: Soldiers*0.3', True, BLACK)
            ...
            for i in range(mapSize * 2 - isEven + 1):
                ...
                if typingPositionB == i:
                    pygame.draw.polygon(self.screen, GREEN, transformToSquare(int(self.length / 2 - mapSize * cubeWidth + cubeWidth * (i)),
                                        int(self.width / 2 - cubeWidth / 2 + 2*cubeWidth)+self.downward, radius), 0)
                else:
                    pygame.draw.polygon(self.screen, BLACK, transformToSquare(
                        int(self.length / 2 - mapSize * cubeWidth + cubeWidth * (i)),
                        int(self.width / 2 - cubeWidth / 2 + 2 * cubeWidth)+self.downward, radius), self.thickness)
...
def calculateRemainingSoldiers(policyA, policyB, remainingSoldiersA, remainingSoldiersB, warField, soldierFromWarFieldA, soldierFromWarFieldB, soldierFromBaseA, soldierFromBaseB):
    ...
    for i in range(len(policyA)):
        remainingSoldiersNew2A[i] = math.ceil((remainingSoldiersNew2A[i]) * (1 - (1 / (len(policyA)-1 + 1 - i))))
        remainingSoldiersNew2B[i] = math.ceil((remainingSoldiersNew2B[i]) * (1 - (1 / (len(policyA)-1 - 6 + i))))
...
class DrawResults:
    ...
    def __call__(self, step, warField, isAutoPeace, remainingSoldiersA, remainingSoldiersB, trial, show):
...
def calculateNumOfWar(warField, policyA, policyB):
    if (policyA.count(1) > 0 and policyA.index(1) >= warField.index(2)) or (
            policyB.count(1) > 0 and policyB.index(1) <= warField.index(1)):
        return 1
    else:
        return 0

def judgeShow(warField, isAutoPeace):
    if isAutoPeace:
        return 3
