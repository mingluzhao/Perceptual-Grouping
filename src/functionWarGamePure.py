...
backgroundList = [(255, 250, 240), BLUE, RED]
...
def calculateRemainingSoldiers(policyA, policyB, remainingSoldiersA, remainingSoldiersB, warField,
    ...
    #gather new soldiers for the next round
    ...
    #cost for logistics
    for i in range(len(remainingSoldiersNew2A)):
        remainingSoldiersNew2A[i] = math.ceil((remainingSoldiersNew2A[i]) * (1 - (1 / (len(policyA) - i))))
        remainingSoldiersNew2B[i] = math.ceil((remainingSoldiersNew2B[i]) * (1 - (1 / (len(policyA) - 7 + i))))
